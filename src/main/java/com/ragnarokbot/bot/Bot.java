...
public class Bot {
	...
	public double calcularDistanciaCentro(MatOfPoint monstro) {
        ...
        return Math.sqrt(Math.pow(boundingRect.x - coordenadasJogadorTelaX, 2) + Math.pow(boundingRect.y - coordenadasJogadorTelaY, 2));
    }
	
	public void moverPersonagem(Coordenadas atual, Coordenadas destino) throws Exception {
        int xMouse = this.coordenadasJogadorTelaX + (destino.x - atual.x) * 4;
        int yMouse = this.coordenadasJogadorTelaY - (destino.y - atual.y) * 4;
